...
<!-- breadcrumb -->
@section('PageTitle')
    {{trans('main_sidebar.processing_fee')}}
@stop
<!-- breadcrumb -->
@endsection
@section('content')
                    ...
                    <table id="datatable" class="table table-striped table-bordered p-0">
                        ...
                        <tbody>
                            ...
                            @foreach($all_processing as $processing)
                                <tr>
                                    <td>{{$i++}}</td>
                                    <td>{{$processing->getStudent->student_name}}</td>
                                    <td>{{number_format($processing->amount, 2)}} $</td>
                                    <td>{{$processing->notes}}</td>
                                    <td>
                                        <a class="modal-effect btn btn-info" href="{{ url('processing_fees/'.$processing->id.'/edit') }}"
                                           title="تعديل"><i class="fa fa-edit"></i>
                                        </a>
                                        <a class="modal-effect btn btn-danger" data-effect="effect-scale"
                                           data-id="{{ $processing->id }}" data-student_name="{{ $processing->getStudent->student_name }}"
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content modal-content-demo">
                <div class="modal-header">
                    <h6 class="modal-title">حذف رسم مستبعد</h6>
