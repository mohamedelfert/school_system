        ...
        <div class="card card-statistics h-100">
            <div class="card-body">
                <div style="margin-bottom: 10px;">
                    <a type="button" class="btn btn-primary" href="{{route('online_classes.create')}}">
                            <i class="fa fa-plus-circle"></i> اضافه حصه جديده
                    </a>
                    <a type="button" class="btn btn-info" href="{{route('indirect.indirectCreate')}}">
                ...
                <div class="table-responsive">
                    <table id="datatable" class="table table-striped table-bordered p-0 text-center">
                        ...
                        <tbody>
                            <?php $i = 1; ?>
                            @foreach($all_online_classes as $online_class)
                                <tr>
                                    <td>{{$i++}}</td>
                                    <td>{{$online_class->getGrades->name}}</td>
                                    <td>{{$online_class->getChapters->chapter_name}}</td>
                                    <td>{{$online_class->getSections->section_name}}</td>
                                    <td>{{$online_class->getUsers->name}}</td>
                                    <td>{{$online_class->topic}}</td>
                                    <td>{{$online_class->start_at}}</td>
                                    <td>{{$online_class->duration}}</td>
                                    <td>
                                        <span class="badge badge-pill badge-primary">
                                            <a href="{{$online_class->join_url}}" target="_blank">انضم الان</a>
                                        </span>
                                    </td>
                                    <td>
                                        @if($online_class->integration == true)
                                            <a class="btn btn-info btn-sm" href="{{route('online_classes.edit',$online_class->meeting_id)}}" title="تعديل">
                                                <i class="fa fa-edit"></i>
                                            </a>
                                        @elseif($online_class->integration == false)
                                            <a class="btn btn-warning btn-sm" href="{{route('online_classes.edit',$online_class->meeting_id)}}" title="تعديل">
                                                <i class="fa fa-edit"></i>
                                            </a>
                                        @endif
                                        <a class="modal-effect btn btn-danger btn-sm" data-effect="effect-scale"
                                           data-id="{{ $online_class->id }}" data-meeting_id="{{ $online_class->meeting_id }}"
                                           data-topic="{{ $online_class->topic }}" data-toggle="modal" href="#delete"
