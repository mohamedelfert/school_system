...
<!-- breadcrumb -->
@section('PageTitle')
    {{trans('main_sidebar.payment_students')}}
@stop
<!-- breadcrumb -->
@endsection
@section('content')
                    ...
                    <table id="datatable" class="table table-striped table-bordered p-0">
                        ...
                        <tbody>
                            ...
                            @foreach($all_payments as $payment)
                                <tr>
                                    <td>{{$i++}}</td>
                                    <td>{{$payment->getStudent->student_name}}</td>
                                    <td>{{number_format($payment->amount, 2)}} $</td>
                                    <td>{{$payment->notes}}</td>
                                    <td>
                                        <a class="modal-effect btn btn-info" href="{{ url('payments_students/'.$payment->id.'/edit') }}"
                                           title="تعديل"><i class="fa fa-edit"></i>
                                        </a>
                                        <a class="modal-effect btn btn-danger" data-effect="effect-scale"
                                           data-id="{{ $payment->id }}" data-student_name="{{ $payment->getStudent->student_name }}"
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content modal-content-demo">
                <div class="modal-header">
                    <h6 class="modal-title">حذف سند صرف</h6>
                    <button aria-label="Close" class="close" data-dismiss="modal" type="button"><span aria-hidden="true">&times;</span></button>
                </div>
                <form action="{{route('payments_students.destroy','test')}}" method="post">
                    {{ method_field('delete') }}
                    {{ csrf_field() }}
